...
> "회의 대본 던지면 → 회의록 정리해서 Confluence에 올리고 → 할 일은 TMT/OPS Jira 티켓으로 끊어준다."
...
- 회의록의 액션 아이템을 Jira(TMT/OPS) 티켓으로 쪼개고 싶을 때
...
옵션: `--epic TMT-9`, `--team-lead <이름>`, `--space <키>`
...
| 게이트 | 시점 | 확인 내용 |
|---|---|---|
| **B-G1** | 액션 추출 후 | 티켓 분리 방식·TMT/OPS 라우팅·담당자/기한 |
...
- OPS 이슈를 TMT Epic 하위(parent)로 넣지 않음 → OPS에 만들고 필요 시 `Relates`로 링크.
...
- **프로젝트 라우팅**: 제품·UX·구현·QA → **TMT** / 운영·일정·문서·커뮤니케이션 → **OPS** (`references/spec.md`).
