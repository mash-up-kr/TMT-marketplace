...
2. 작성 전에 기존 Confluence 회의록 컨벤션을 학습:
   - `mcp__atlassian__searchConfluenceUsingCql`로 `title ~ "회의록" OR title ~ "meeting" OR label = "meeting-minutes"` 같은 작은 쿼리 사용.
   - `mcp__atlassian__getConfluencePage(contentFormat=markdown)`로 관련 상위 3-5개 페이지를 가져옴.
