...
4. 담당자 정규화:
   - sooperset는 `assignee`에 **displayName**을 넣습니다(accountId 아님). `references/people.md`의 displayName 사용.
   - watcher 추가용으로는 같은 표의 accountId를 사용(아래 9번 `jira_add_watcher`).
...
9. watcher는 `mcp__atlassian__jira_add_watcher(issue_key=, user_id=<accountId>)`로 추가(displayName 아님). 소스/맥락 코멘트가 필요하면 `mcp__atlassian__jira_add_comment(issue_key=, comment=<markdown>)`로 추가.
